...
export const Route = createFileRoute("/products/$categorySlug/$seriesSlug/")({
  component: SeriesRouteHandler,
});

function SeriesRouteHandler() {
  const { categorySlug, seriesSlug } = Route.useParams();
  
  // Independent and highly reliable lookups for SSR stability
    ...
    return (
      <SiteLayout>
        <div className="flex min-h-[50vh] flex-col items-center justify-center bg-background px-4 py-12 text-center">
          <h2 className="font-display text-3xl font-bold text-primary">Series Not Found</h2>
          <p className="mt-2 text-muted-foreground">The product series you are looking for does not exist.</p>
          <Link
            to="/products"
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 max-w-3xl">
            ...
            {(series.description || series.short) && (
              <p className="mt-4 text-base sm:text-lg text-primary-foreground/75 leading-relaxed max-w-2xl">
                {series.description || series.short}
              </p>
            )}
          </div>
